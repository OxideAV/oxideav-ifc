...
- Phase 3: `IfcSectionedSurface` — open profiles lofted at linear
  ...
  sectioned surface.
- WHERE rules for `IfcBooleanResult` / `IfcBooleanClippingResult`
  (`IfcSweptDiskSolid` accepted despite the schema's DISC literal, digest
  §5.3), `IfcPolygonalBoundedHalfSpace` and `IfcBoxedHalfSpace`.
...
- Half-spaces on `IfcCylindricalSurface` / `IfcSphericalSurface` bases
  ...
  materialised over the operand's extent (previously `Unsupported`).
...
- `fuzz/` cargo-fuzz crate with the `tessellate` target and a seed corpus;
...
- Curved Breps carve as Boolean operands (half-space clipping of an
  advanced-Brep cylinder pinned watertight at half volume).

## [0.0.2](https://github.com/OxideAV/oxideav-ifc/compare/v0.0.1...v0.0.2) - 2026-08-30

### Other

- README + crate docs: Phase-3 truth after named profiles, tapered sweeps, rules, georeferencing
- EXPRESS WHERE-rule validation for profiles, swept solids and georeferencing
- platform-stable fillet arc segment count
- :transform + synthetic steelwork fixture test
- drop the unused extrude_area wrapper left by the tapered loft
- Phase 3: IfcExtrudedAreaSolidTapered + IfcRevolvedAreaSolidTapered
- Phase 3: IfcDerivedProfileDef + IfcMirroredProfileDef
- Scale on all three axes, IfcMapConversionScaled, IfcRigidOperation; prefixed-derived-unit opt-in policy
- Rectangle-hollow fillet radii; typed schema for named profiles, tapered/surface-curve sweeps, IfcMapConversionScaled, IfcRigidOperation
- Phase 3: named parameterised profiles (I/L/T/U/Z/C, rounded rectangle, trapezium)
